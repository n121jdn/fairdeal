...
def extract_features(
    ...
    """
    Build the feature vector for one order.
    All temporal features are derived from created_at so there is
    no dependency on order_delivered_time at inference time.
    """
    return np.array([[
        distance_km,
        items_count,
...
class DeliveryPredictor:
    ...
    def train(self, df) -> dict:
        ...
        Train on a pandas DataFrame with columns:
            distance_km, items_count, order_created_time (datetime),
            queue_at_dispatch (int), order_delivered_time (datetime)

        Returns a metrics dict with MAE, R², and CV-MAE.
        """
        import pandas as pd
        ...
        df = df[df["actual_delivery_minutes"].between(
    ...
    def predict(
        ...
        """
        Return predicted delivery time in minutes for a single order.
        Falls back to _FALLBACK_DELIVERY_MINS if no model is trained.
        """
    ...
    def load(self) -> bool:
        ...
        logger.info(f"DeliveryPredictor loaded from {MODEL_PATH} | metrics={self._metrics}")
        return True
